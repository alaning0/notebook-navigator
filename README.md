> **Fork note:** This is [alaning0/notebook-navigator](https://github.com/alaning0/notebook-navigator), forked from [johansan/notebook-navigator](https://github.com/johansan/notebook-navigator). It keeps the same plugin id (`notebook-navigator`) so it can override a Community Plugin install. Upstream is tracked as the `upstream` remote. Calendar agenda (month + year + event list) lives on the `calendar-agenda` branch.
